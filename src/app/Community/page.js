...
export default function Community() {
    const [postContent, setPostContent] = useState(""); // Store post description
    ...
    return (
        <main>
            ...
            <div className="container mx-auto bg-gray-200 -z-10 rounded-2xl text-2xl p-8">
                ...
                {/*Popup content for user to create a post*/}
                <Popup 
                    trigger={ButtonPopup} 
                    ...
                    {posts.map((post) => (
                        <div key={post.id} className="bg-white p-4 my-4 rounded-lg shadow-md">
                            <div className="flex items-center space-x-1 mb-4">
                                    ...
                                    />
                                <p className="font-sans italic text-[20px]">{"@" + post.username || "Anonymous"}</p>
                            </div>
                            {/*NEW: Display Posts' Title */}
                            <h4>{post.title}</h4>
                            <p>{post.content}</p>
